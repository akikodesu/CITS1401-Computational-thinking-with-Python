...
#Q13: Write a function should_shutdown(battery_level, time_on) which returns True when time_on is less than 60 and battery_level is less than 4.7. If time_on is greater than or equal to 60, return True if battery_level is less than 4.8. In all other situations the function returns False.
...
#Q14: The greatest common divisor (GCD) of two positive integers is the largest integer that evenly divides into both of them. For example, the GCD of 18 and 12 is 6.
"""
A general algorithm to calculate GCDs is as follows:

1. Declare a variable and initialize with 1 (lets call this number stored in this variable the iterative number).
2. Check whether both the input numbers are divisible by the iterative number. If they are then save the iterative number to a different variable (you only need to keep track of the current largest iterative number that divides both inputs in this variable). 
3. Increment the iterative number and repeat step 2 until the iterative number is equal to the smaller input number: at this point the largest iterative number which divides both the inputs numbers is the GCD (you stored this number in a different variable as asked to in step 2 right?).

Write a function gcd(num1,num2) that takes two integers and return GCD of the two input integers. You can assume that inputs will always be positive integers.
"""
def gcd(num1, num2):
    if num1 == num2:
        return num1
    ...
    while iterative_num <= smaller_num:
        if num1 % iterative_num == 0 and num2 % iterative_num == 0:
            gcd_num = iterative_num
        iterative_num += 1

    return gcd_num


#Q15: A bit shift is a procedure that takes a binary string and moves the digits either left or right, wrapping digits around as required; e.g. a left shift of 1 applied to 10011 gives 00111, and a left-shift of 2 gives 01110.
#Write a function bitshift(s,k,b) that takes a string s, an integer k, and a Boolean b, and returns s shifted by k, left if b is True, and right otherwise.
def bitshift(s, k, b):
    if b:
        shifted_s = s[k:] + s[:k]
    else:
...
#Q16: Assume binary represents an unsigned binary number of arbitrary length. Write a function binarytodecimal(binary) to calculate the decimal equivalent of binary. Pass the binary number as a parameter of type str (string) to the function.
def binarytodecimal(binary):
    decimal = 0
    length = len(binary)
    
    for i in range(length):
        x = int(binary[length - i - 1])
        decimal += x * (2 ** i)
